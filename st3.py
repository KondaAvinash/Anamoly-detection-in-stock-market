...
# Try loading ticker list from pickle; fallback to hardcoded list if file doesn't exist
try:
    with open("tickers.pickle", "rb") as f:
        ticker_list = pickle.load(f)
except FileNotFoundError:
    print("tickers.pickle not found. Using predefined list of tickers.")
    # Fallback to a predefined list of tickers
    ticker_list = ["AAPL", "GOOGL", "MSFT", "AMZN", "TSLA", "FB", "NFLX", "BA", "DIS", "NVDA"]
...
def graph_generator(n_clicks, ticker, chart_name):
    ...
    if n_clicks >= 1:  # Checking for user to click submit button
        ...
        df['close'] = df['close'] * 1.1  # Adjusting the closing price for demo purpose
        stock = Sdf(df)
